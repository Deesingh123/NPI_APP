...
def main():
    ...
    #st.title("MERLIN - Process Readiness Tracker")

    REFRESH_INTERVAL = 30
    CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQBqDIx_ZBSYN7RaWxCIjHMZeFBkMhQaKcmc8mvq9KrE-Z1EFeaIsC1B4Fmw_wE_1NbzsConI04b6o0/pub?gid=398221268&single=true&output=csv"  # Update with real MERLIN link
    ...
    <div style="text-align:center; padding:20px; background:linear-gradient(135deg, #1d4ed8 0%, #3b82f6 100%); color:white; border-radius:16px; margin-bottom:15px; box-shadow: 0 12px 30px rgba(29,78,216,0.3);">
        <h1 style="margin:0; font-size:2.4rem; font-weight:800;">MERLIN</h1>
    ...
    # Timeline Header (from your image) - Beautiful Gantt-style bar
    # Professional Timeline Bar - Perfect Match to Your Image
    # Perfect Timeline Bar - Exact Match to Your Image
    # Project Timeline - Clean & Professional
    st.markdown("""
    ...
    # Compact Metric Cards - Reduced Vertical Height
    # Compact Metric Cards - Perfect Match to Your Image
    ...
    with col1:
        if owner_col:
            ...
            chosen_owner = st.selectbox("👤 Owner", owners, key="owner_ready_merlin")
            if chosen_owner != "All":
                filtered = filtered[filtered[owner_col] == chosen_owner]

    with col2:
        categories = ["All"] + sorted(filtered[category_col].dropna().unique().tolist())
        chosen_cat = st.selectbox("📋 Process Category", categories, key="cat_ready_merlin")
        if chosen_cat != "All":
            filtered = filtered[filtered[category_col] == chosen_cat]

    with col3:
        view = st.selectbox("🔍 View", ["All Items", "Only Delayed", "Only Open", "Only Closed"], key="view_ready_merlin")
